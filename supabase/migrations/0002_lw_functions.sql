-- ═══════════════════════════════════════════════════════════════
--  live-worksheet · 0002 서버 함수
--
--  브라우저(publishable key)의 쓰기는 모두 여기의 security definer 함수를 거친다.
--  검사는 코드에 박힌 이름이 아니라 **등록된 연수 설정(lw_events.config)**을 읽어서 한다.
...
-- 실패 응답 한 줄
create or replace function public.lw_fail(p_code text, p_msg text)
returns jsonb language sql immutable set search_path = public, pg_temp as $$
  select jsonb_build_object('ok', false, 'code', p_code, 'msg', p_msg);
$$;

-- 앞뒤 공백만 뗀다(전각 공백·줄 바꿈 없는 공백 포함). 가운데는 건드리지 않는다.
create or replace function public.lw_trim(p text)
returns text language sql immutable set search_path = public, pg_temp as $$
  select regexp_replace(coalesce(p, ''), '^[\s 　﻿]+|[\s 　﻿]+$', '', 'g');
$$;

-- 한 줄 입력 정리: 앞뒤 공백을 떼고 안쪽의 연속 공백·줄 바꿈을 공백 하나로
create or replace function public.lw_one_line(p text)
returns text language sql immutable set search_path = public, pg_temp as $$
  select regexp_replace(public.lw_trim(p), '[\s 　]+', ' ', 'g');
$$;

-- 문자열 → uuid (형식이 틀리면 null)
create or replace function public.lw_to_uuid(p text)
...
create or replace function public.lw_admin_ok(p_event_id text, p_passcode text)
returns boolean language sql stable security definer set search_path = public, pg_temp as $$
  select exists (
    select 1 from public.lw_event_secrets s
...
create or replace function public.lw_participant_responses(p_participant_id uuid)
returns jsonb language sql stable security definer set search_path = public, pg_temp as $$
  select coalesce(jsonb_object_agg(r.activity_id, r.payload), '{}'::jsonb)
    from public.lw_responses r where r.participant_id = p_participant_id;
$$;

-- ───────────────────────────────────────────────
--  활동 종류별 서버 검사 (부품 규약의 '서버 검사 규칙')
--  돌려주는 형식: {ok:true, payload:<정리된 payload>} / {ok:false, msg}
--  새 종류를 더할 때: lw_validate_<종류> 함수를 만들고 lw_validate_payload 에 한 줄 더한다.
-- ───────────────────────────────────────────────

-- ox: { "answers": ["O"|"X", …] } 길이 = 문항 수
create or replace function public.lw_validate_ox(p_activity jsonb, p_payload jsonb)
...
create or replace function public.lw_validate_stage_check(p_activity jsonb, p_payload jsonb)
returns jsonb language plpgsql immutable set search_path = public, pg_temp as $$
declare
...
begin
  ...
  if v_obj_id = 'custom' then
    ...
    v_obj_text := public.lw_one_line(v_obj->>'text');
  ...
  for r in select key, value from jsonb_each(v_items) loop
    ...
    if v_t is null or v_t = 'null' then
      ...
      v_memo := public.lw_one_line(r.value->>'memo');
...
create or replace function public.lw_validate_sentence(p_activity jsonb, p_payload jsonb)
returns jsonb language plpgsql immutable set search_path = public, pg_temp as $$
declare
  v_tpl   text;
  v_blank text;
begin
  ...
  v_blank := public.lw_one_line(p_payload->>'blank');
...
create or replace function public.lw_validate_payload(p_activity jsonb, p_payload jsonb)
returns jsonb language plpgsql immutable set search_path = public, pg_temp as $$
begin
  ...
  case p_activity->>'type'
    when 'ox'          then return public.lw_validate_ox(p_activity, p_payload);
    when 'stage_check' then return public.lw_validate_stage_check(p_activity, p_payload);
    when 'sentence'    then return public.lw_validate_sentence(p_activity, p_payload);
...
create or replace function public.lw_get_event(p_event_id text)
returns jsonb language plpgsql stable security definer set search_path = public, pg_temp as $$
declare
  v_ev       public.lw_events%rowtype;
  v_settings jsonb;
  v_reveal   jsonb;
begin
  select * into v_ev from public.lw_events where id = p_event_id;
  if not found then
    return public.lw_fail('no_event', '연수를 찾을 수 없습니다.');
  end if;

  select coalesce(jsonb_object_agg(s.key, s.value), '{}'::jsonb) into v_settings
    from public.lw_settings s where s.event_id = v_ev.id;
  ...
  select coalesce(jsonb_object_agg(a->>'id', sec.reveal->(a->>'id')), '{}'::jsonb) into v_reveal
    ...
    join public.lw_event_secrets sec on sec.event_id = v_ev.id
   where a->>'type' = 'ox'
...
create or replace function public.lw_join(p_event_id text, p_name text, p_mode text default 'check')
returns jsonb language plpgsql volatile security definer set search_path = public, pg_temp as $$
declare
  v_raw  text := public.lw_trim(p_name);
  v_mode text := coalesce(nullif(p_mode, ''), 'check');
  v_norm text;
  v_row  public.lw_participants%rowtype;
begin
  if not exists (select 1 from public.lw_events where id = p_event_id) then
    return public.lw_fail('no_event', '연수를 찾을 수 없습니다.');
  end if;

  if v_raw <> '' and public.lw_admin_ok(p_event_id, v_raw) then
  ...
  if v_mode not in ('check', 'resume', 'new') then
    return public.lw_fail('bad_mode', '입장 방식이 올바르지 않습니다.');
  end if;
  if char_length(v_raw) < 1 then
    return public.lw_fail('bad_name', '이름을 입력해 주세요.');
  end if;
  if char_length(v_raw) > 20 then
    return public.lw_fail('bad_name', '이름은 20자 이내로 입력해 주세요.');
  end if;

  v_norm := lower(regexp_replace(v_raw, '\s+', ' ', 'g'));

  select * into v_row from public.lw_participants
  ...
  if found and v_mode = 'resume' then
    update public.lw_participants set last_seen = now() where id = v_row.id;
    return jsonb_build_object('ok', true, 'resumed', true,
                              'participant', jsonb_build_object('id', v_row.id, 'name', v_row.name),
                              'responses', public.lw_participant_responses(v_row.id));
  end if;

  insert into public.lw_participants (event_id, name, norm_name)
...
create or replace function public.lw_restore(p_event_id text, p_participant_id text)
returns jsonb language plpgsql volatile security definer set search_path = public, pg_temp as $$
declare
  v_pid uuid := public.lw_to_uuid(p_participant_id);
  v_row public.lw_participants%rowtype;
begin
  if not exists (select 1 from public.lw_events where id = p_event_id) then
    return public.lw_fail('no_event', '연수를 찾을 수 없습니다.');
  ...
  if v_pid is null or not found then
    return public.lw_fail('no_participant', '참가자 정보를 찾을 수 없습니다. 새로고침해 주세요.');
  end if;
  update public.lw_participants set last_seen = now() where id = v_row.id;
...
create or replace function public.lw_submit(p_event_id text, p_participant_id text,
...
begin
  ...
  if v_pid is null or not exists (select 1 from public.lw_participants
  ...
  if v_act is null then
    return public.lw_fail('no_activity', '알 수 없는 활동입니다.');
  end if;

  if coalesce((select s.value from public.lw_settings s
                where s.event_id = p_event_id and s.key = 'open:' || p_activity_id), 'N') <> 'Y' then
    return public.lw_fail('closed', '아직 열리지 않은 과제입니다.');
  end if;

  if p_payload is null or jsonb_typeof(p_payload) <> 'object' or pg_column_size(p_payload) > 16384 then
    return public.lw_fail('invalid', '응답 형식이 올바르지 않습니다.');
  end if;

  v_res := public.lw_validate_payload(v_act, p_payload);
  if (v_res->>'ok')::boolean is not true then
    return public.lw_fail('invalid', coalesce(v_res->>'msg', '응답 형식이 올바르지 않습니다.'));
  end if;

  insert into public.lw_responses (event_id, activity_id, participant_id, payload)
  values (p_event_id, p_activity_id, v_pid, v_res->'payload')
  on conflict (event_id, activity_id, participant_id)
  do update set payload = excluded.payload, updated_at = now();

  update public.lw_participants set last_seen = now() where id = v_pid;
  return jsonb_build_object('ok', true);
end $$;

-- 관리자 · 진행 설정 바꾸기
--   키: open:<이 연수의 활동 id> · reveal:<이 연수의 ox 활동 id> · materials_open, 값: Y/N
create or replace function public.lw_admin_set(p_event_id text, p_key text, p_value text, p_passcode text)
returns jsonb language plpgsql volatile security definer set search_path = public, pg_temp as $$
declare
  v_ev public.lw_events%rowtype;
  v_ok boolean;
begin
  ...
  if not public.lw_admin_ok(p_event_id, p_passcode) then
    return public.lw_fail('auth', '관리자 인증에 실패했습니다.');
  end if;
  if p_value is null or p_value not in ('Y', 'N') then
    return public.lw_fail('bad_value', '값은 Y 또는 N만 쓸 수 있습니다.');
  end if;
  ...
  if v_ok is not true then
    return public.lw_fail('bad_key', '알 수 없는 설정입니다.');
  end if;

  insert into public.lw_settings (event_id, key, value) values (p_event_id, p_key, p_value)
...
create or replace function public.lw_admin_reset(p_event_id text, p_passcode text)
returns jsonb language plpgsql volatile security definer set search_path = public, pg_temp as $$
declare
  v_r int;
  v_p int;
begin
  ...
  delete from public.lw_responses where event_id = p_event_id;
  get diagnostics v_r = row_count;
  delete from public.lw_participants where event_id = p_event_id;
  get diagnostics v_p = row_count;
  return jsonb_build_object('ok', true, 'participants', v_p, 'responses', v_r);
end $$;

-- 관리자 확인 (관리자 화면을 다시 열 때 기기에 남은 암호로 확인)
create or replace function public.lw_admin_check(p_event_id text, p_passcode text)
returns jsonb language sql stable security definer set search_path = public, pg_temp as $$
  select jsonb_build_object('ok', public.lw_admin_ok(p_event_id, p_passcode));
$$;

-- 깨우기용 가벼운 조회
create or replace function public.lw_ping()
...
--  예약 작업(lw_anonymize, 0003)이 매일 부른다. 여러 번 불러도 결과가 같다.
-- ───────────────────────────────────────────────
create or replace function public.lw_anonymize_expired()
returns integer language plpgsql volatile security definer set search_path = public, pg_temp as $$
declare v_n int;
begin
  update public.lw_participants p
     set name = '익명',
         norm_name = 'anon:' || p.id::text,   -- 식별 불가·고유 값(20자 이름과 겹칠 수 없다)
         anonymized_at = now()
    from public.lw_events e
...
revoke all on function public.lw_fail(text, text)                           from public, anon, authenticated;
revoke all on function public.lw_trim(text)                                 from public, anon, authenticated;
revoke all on function public.lw_one_line(text)                             from public, anon, authenticated;
revoke all on function public.lw_to_uuid(text)                              from public, anon, authenticated;
revoke all on function public.lw_admin_ok(text, text)                       from public, anon, authenticated;
revoke all on function public.lw_participant_responses(uuid)                from public, anon, authenticated;
revoke all on function public.lw_validate_ox(jsonb, jsonb)                  from public, anon, authenticated;
revoke all on function public.lw_validate_stage_check(jsonb, jsonb)         from public, anon, authenticated;
revoke all on function public.lw_validate_sentence(jsonb, jsonb)            from public, anon, authenticated;
revoke all on function public.lw_validate_payload(jsonb, jsonb)             from public, anon, authenticated;
revoke all on function public.lw_anonymize_expired()                        from public, anon, authenticated;

-- 공개 서버 함수
revoke all on function public.lw_get_event(text)                            from public;
revoke all on function public.lw_join(text, text, text)                     from public;
revoke all on function public.lw_restore(text, text)                        from public;
revoke all on function public.lw_submit(text, text, text, jsonb)            from public;
revoke all on function public.lw_admin_set(text, text, text, text)          from public;
revoke all on function public.lw_admin_reset(text, text)                    from public;
revoke all on function public.lw_admin_check(text, text)                    from public;
revoke all on function public.lw_ping()                                     from public;
grant execute on function public.lw_get_event(text)                         to anon, authenticated;
grant execute on function public.lw_join(text, text, text)                  to anon, authenticated;
grant execute on function public.lw_restore(text, text)                     to anon, authenticated;
grant execute on function public.lw_submit(text, text, text, jsonb)         to anon, authenticated;
grant execute on function public.lw_admin_set(text, text, text, text)       to anon, authenticated;
grant execute on function public.lw_admin_reset(text, text)                 to anon, authenticated;
grant execute on function public.lw_admin_check(text, text)                 to anon, authenticated;
grant execute on function public.lw_ping()                                  to anon, authenticated;

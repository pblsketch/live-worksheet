 ...
 *   - lw_events · lw_event_secrets 를 넣거나 갱신한다(관리자 암호는 DB에서 pgcrypto로 해시).
...
const sql = `
with ev as (
  insert into public.lw_events (id, title, date, listed, config)
  ...
  insert into public.lw_event_secrets (event_id, admin_hash, reveal)
  ...
  insert into public.lw_settings (event_id, key, value)
...
select (select count(*) from sec)::int as secrets,
       ...
       coalesce((select json_agg(key order by key) from public.lw_settings

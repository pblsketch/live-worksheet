 ...
 *   - 적용 전 기준 목록(supabase/baseline-snapshot.json)과 비교해 lw_ 아닌 객체가 생기거나 사라지지 않았다
...
const TABLES = ['lw_events', 'lw_event_secrets', 'lw_participants', 'lw_responses', 'lw_settings'];
const PUBLIC_RPC = ['lw_get_event', 'lw_join', 'lw_restore', 'lw_submit', 'lw_admin_set', 'lw_admin_reset',
  'lw_admin_check', 'lw_ping'];

describe('스키마', () => {
  let snap;

  it('기준 목록 대비 lw_ 아닌 객체의 추가·삭제가 없다', async () => {
  ...
  it('표 다섯 개, 공개 서버 함수, 발행, 예약 작업이 있다', () => {
    ...
    for (const t of ['lw_participants', 'lw_responses', 'lw_settings']) {
      assert.ok(snap.includes(`publication:supabase_realtime:${t}`), `실시간 ${t}`);
    }
    assert.ok(snap.includes('cron:lw_anonymize'));
  });
  ...
  it('RLS 가 켜져 있고, 실시간 표는 replica identity full 이다', async () => {
    const rows = await runSql(`
      select c.relname, c.relrowsecurity, c.relreplident
       ...
       where n.nspname = 'public' and c.relname like 'lw\\_%' and c.relkind = 'r'`);
    const m = Object.fromEntries(rows.map((r) => [r.relname, r]));
    for (const t of TABLES) assert.equal(m[t].relrowsecurity, true, `${t} RLS`);
    for (const t of ['lw_participants', 'lw_responses', 'lw_settings']) assert.equal(m[t].relreplident, 'f', `${t} replica identity`);
  });
  ...
  it('anon 권한: 네 표는 읽기만, 비밀 표는 아무것도, 서버 함수는 공개 목록만 실행', async () => {
    const rows = await runSql(`
      select t.t as tbl,
        ...
        from unnest(array['lw_events','lw_event_secrets','lw_participants','lw_responses','lw_settings']) as t(t)`);
    for (const r of rows) {
      assert.equal(r.sel, r.tbl !== 'lw_event_secrets', `${r.tbl} select`);
      assert.deepEqual([r.ins, r.upd, r.del, r.trn], [false, false, false, false], `${r.tbl} 쓰기 권한`);
    }
    const fns = await runSql(`
      select p.proname from pg_proc p join pg_namespace n on n.oid = p.pronamespace
       where n.nspname = 'public' and p.proname like 'lw\\_%' and has_function_privilege('anon', p.oid, 'execute')
       order by 1`);
  ...
  it('공개 서버 함수는 security definer 이고 search_path 가 고정되어 있다', async () => {
    const rows = await runSql(`
      select p.proname, p.prosecdef, p.proconfig from pg_proc p join pg_namespace n on n.oid = p.pronamespace
       where n.nspname = 'public' and p.proname like 'lw\\_%'`);
    for (const r of rows) {
      assert.ok((r.proconfig || []).some((c) => c.startsWith('search_path=')), `${r.proname} search_path`);
      if (PUBLIC_RPC.includes(r.proname) && r.proname !== 'lw_ping') assert.equal(r.prosecdef, true, `${r.proname} security definer`);
    }
  });
});
